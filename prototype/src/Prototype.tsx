...
type SheetName = "supplier" | "models" | "attach" | "tools" | "projects" | "settings" | null;
  ...
  return (
    <>
      <MobileScroll className="chida-app auth-page">
        <main className="auth-shell" dir="rtl" data-theme={theme} data-testid="auth-flow">
          <AuthHeader
      ...
      <BottomSheet
        ...
        snap={0.36}
      >
        <div className="sheet-message" dir="rtl" data-theme={theme} data-testid="supplier-unavailable-sheet">
...
function AuthHeader({ canGoBack, onBack, theme, toggleTheme }: { canGoBack: boolean; onBack: () => void; theme: ThemeName; toggleTheme: () => void }) {
  return (
    <header className="auth-header">
      ...
      <div className="auth-header-actions">
        <button className="icon-button" type="button" onClick={toggleTheme} aria-label="تغییر پوسته" data-testid="theme-toggle">
...
function BuilderHome({ activeProject, modelMode, onProjectChange, onModelChange, onOpenSheet, sheet, theme, toggleTheme }: { activeProject: string; modelMode: ModelMode; onProjectChange: (project: string) => void; onModelChange: (mode: ModelMode) => void; onOpenSheet: (sheet: SheetName) => void; sheet: SheetName; theme: ThemeName; toggleTheme: () => void }) {
  const keyboard = useKeyboard();
  ...
  return (
    <div className="chida-app chida-shell" dir="rtl" data-theme={theme} data-testid="builder-home">
      ...
      <header className="app-header">
        ...
        <button className="profile-button" type="button" onClick={() => onOpenSheet("settings")} aria-label="پروفایل و تنظیمات" data-testid="profile-button"><span>م</span></button>
      </header>
      ...
      <section className="composer-dock" style={{ bottom: bottomInset + 8 }} data-testid="composer-dock">
        ...
        <div className="composer-stack" data-testid="composer-box">
          <div className="composer-card" data-testid="composer-card">
            ...
            <div className="composer-actions">
              ...
              <div className="composer-action-group send-tools">
                ...
                <button className="send-button" type="button" onClick={sendMessage} aria-label="ارسال پیام" data-testid="send-button" data-ready={draft.trim() ? "true" : "false"}><Send size={19} /></button>
              </div>
            </div>
          </div>
          <div className="project-context" data-testid="project-context">
            ...
            <button className="tool-cluster" type="button" onClick={() => onOpenSheet("tools")} aria-label="ابزارهای فعال" data-testid="capability-cluster"><span><Search size={14} /></span><span><FileText size={14} /></span><span><Wrench size={14} /></span></button>
          </div>
        </div>
      </section>
        ...
        {drawerOpen ? (
          <>
            ...
            <motion.aside className="app-drawer" data-testid="nav-drawer" initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", stiffness: 420, damping: 42 }}>
              ...
              <button className="drawer-profile" type="button" onClick={() => { setDrawerOpen(false); onOpenSheet("settings"); }}>
                <span className="profile-button"><span>م</span></span><span className="drawer-profile-copy"><strong>مهیار کلباسی</strong><small>حساب سازنده</small></span><Settings size={18} />
              </button>
            </motion.aside>
          </>
        ) : null}
      ...
      <ToolsSheet sheet={sheet} onClose={() => onOpenSheet(null)} />
      <ProjectsSheet sheet={sheet} activeProject={activeProject} onClose={() => onOpenSheet(null)} onSelect={onProjectChange} />
      <SettingsSheet sheet={sheet} theme={theme} toggleTheme={toggleTheme} onClose={() => onOpenSheet(null)} />
      <span className="sr-only" aria-live="polite">{projectMeta}</span>
    </div>
  );
}

function SheetRow({ icon, title, description, selected, onClick }: { icon: ReactNode; title: string; description: string; selected?: boolean; onClick: () => void }) {
  return <button className="sheet-row" type="button" onClick={onClick} data-selected={selected ? "true" : "false"}><span className="sheet-row-icon">{icon}</span><span className="sheet-row-copy"><strong>{title}</strong><small>{description}</small></span>{selected ? <Check size={18} /> : null}</button>;
}
...
function ToolsSheet({ sheet, onClose }: { sheet: SheetName; onClose: () => void }) {
  return <BottomSheet open={sheet === "tools"} onOpenChange={(open) => !open && onClose()} title="ابزارهای فعال" description="این ابزارها در زمینهٔ پروژهٔ جاری به چیدا کمک می‌کنند." snap={0.5}><div className="sheet-list" dir="rtl" data-testid="tools-sheet"><SheetRow icon={<Search size={20} />} title="جست‌وجوی بازار" description="بررسی منابع و گزینه‌های قابل استناد" onClick={onClose} /><SheetRow icon={<FileText size={20} />} title="اسناد پروژه" description="نقشه‌ها، صورت‌جلسه‌ها و درخواست‌ها" onClick={onClose} /><SheetRow icon={<Wrench size={20} />} title="دستیار فنی" description="چک‌لیست و تحلیل تخصصی ساخت" onClick={onClose} /></div></BottomSheet>;
}
...
function SettingsSheet({ sheet, theme, toggleTheme, onClose }: { sheet: SheetName; theme: ThemeName; toggleTheme: () => void; onClose: () => void }) {
  return (
    <BottomSheet open={sheet === "settings"} onOpenChange={(open) => !open && onClose()} title="پروفایل و تنظیمات" description="مهیار کلباسی · حساب سازنده" snap={0.68}>
      <div className="settings-sheet" dir="rtl">
        <button className="setting-row" type="button" onClick={toggleTheme} data-testid="settings-theme-toggle"><span className="sheet-row-icon">{theme === "light" ? <Moon size={20} /> : <Sun size={20} />}</span><span><strong>ظاهر برنامه</strong><small>{theme === "light" ? "روشن" : "تیره"}</small></span><span className="theme-toggle" data-on={theme === "dark" ? "true" : "false"}><i /></span></button>
