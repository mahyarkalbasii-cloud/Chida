...
**تاریخ آخرین به‌روزرسانی:** ۱۴۰۵/۰۶/۱۳ — ۲۰۲۶/۰۹/۰۴
**مسیر پروژه:** `/Users/mahyarkl/Desktop/ChatGPT/CHIDA`
**نقطهٔ ادامه:** baseline منتشرشدهٔ زنده `origin/main@f77fb5a22985069028fdaecb0e324375b819ef7b` شامل T9-B1 است و در GitHub، Cloudflare Pages و ChatGPT Sites owner-only به‌صورت same-source تحویل شده است. checkout اصلی روی `main`، design/plan و implementation base بازبینی‌شدهٔ `T9-B2 — Project Input Disposition` و `T9-B3 — Explicit Project Visit Checkpoint` را روی commit دقیق `cb3a619225acfb821abe61dde6abdc4f41c8aee3` به‌علاوهٔ repairهای محدود final review در working candidate دارد؛ ثبت همین سه سند، candidate مشترک را پیش از release منجمد می‌کند. اجرای ۵۶/۵۶ مربوط به candidate پیش از این review-fixهاست و receipt exact final bytes نیست؛ معیار قطعی candidate نهایی همان یک full gate پس از freeze است. دو sidecar هنوز local/unpublished و بدون release receipt هستند. ماهیار انتشار ترکیبی دو تسک را صریحاً مجاز کرده است؛ `BG-GATE-1` تاریخی همچنان `FAIL` و rerun نشده است.

## مأموریت گفت‌وگوی بعدی

**نقطهٔ توقف جاری:** T9-B2/T9-B3 پیاده‌سازی و QA موبایل را گذرانده‌اند. اجرای متمرکز ۵۶/۵۶ در ۳٫۷ دقیقه و build/integrity PASS متعلق به candidate پیش از final whole-slice review بود؛ همان review سه finding محدود Important در focus/copy پیدا کرد. scoped rereview سپس یک follow-on همان کلاس focus را یافت: intent موفق قبلی فقط project+logical key داشت و target fingerprint drift همان پروژه را pin نمی‌کرد. binding نهایی exact target version/fingerprint را نیز می‌سنجد و copy checkpoint preservation-neutral است؛ focused repairها ۴/۴، تکرار race برابر ۲۰/۲۰ و target-drift دوصفحه‌ای ۱/۱ پاس‌اند. rerulingهای نهایی authority و UI هر دو SPEC/QUALITY PASS، Critical صفر و Important صفر هستند. یک Minor پوششی—نبود E2E مستقل failure branch در `BriefInputsSection`—صریحاً non-blocking پذیرفته شده است. این شواهد متمرکز receipt exact final bytes نیستند. Task 7 فقط سه سند وضعیت را هم‌تراز و bytes نهایی را freeze می‌کند. مجوز انتشار از قبل وجود دارد؛ اقدام بعدی اجرای دقیقاً یک `gate:release` کامل روی frozen bytes و—فقط در صورت PASS—commit همان exact gated bytes، سپس `gate:publish`، یک push و انتشار همان SHA در GitHub `main`، Cloudflare Pages و ChatGPT Sites owner-only است. اگر گیت شکست بخورد، receipt وجود ندارد و تغییر اصلاحی candidate تازه و گیت کامل تازه می‌خواهد. پیش از رسید موفق هر سه مقصد هیچ تسک محصولی بعدی آغاز نمی‌شود؛ مدل، backend، شبکه/ارسال، Report/Notification، `case_private` و مسیر تأمین‌کننده خارج از دامنه‌اند.
...
**سابقهٔ blocker گیت انتشار BG-F6؛ در baseline کد `7cab540` بسته‌شده:** نخستین گیت BG-F6 با ۶ شکست از ۴۶۳ متوقف شد. Proposal v2 یک مقدار مرزی معتبر را به‌علت سقف ۱۶۰نویسه‌ای رد می‌کرد و پنج مسیر قدیمی T7/T8 هنوز v1 را به‌شکل آرایه می‌خواندند؛ چند byte oracle دیگر نیز به‌جای authority واقعی null-to-null بودند. حد مشترک canonical به ۲۰۰ هم‌تراز و همهٔ oracleهای غیرمهاجرتی به v2 وصل شدند؛ شش failure برابر ۶/۶ و بستهٔ عمیق برابر ۹۱/۹۱ پاس شد. این اصلاح‌ها اکنون داخل baseline local/GitHub `7cab540ffad3e501e76a75c08a0e6462e36b7cea` هستند؛ گیت ناموفق تاریخی receipt نبود و وضعیت Cloudflare/Sites این SHA در REL-2 دوباره ممیزی نشده است.

**سابقهٔ توقف‌های pre-release T9-B2/T9-B3؛ بسته‌شده پیش از full gate:** اجرای یک‌بارهٔ نخستِ دامنهٔ متمرکز ۵۴/۵۵ بود و روی race فوکوس CTA ثبت checkpoint شکست خورد؛ freeze متوقف شد و آن candidate evidence پذیرش یا receipt نبود. بازتولید قطعی early-rAF ابتدا RED برابر ۰/۱ و سپس GREEN شد؛ stale-focus دو‌تب نیز RED برابر ۰/۱ و پس از repair همراه regression مجاور ۲/۲ پاس شد و checkpointهای مجاور ۵/۵ باقی ماندند. فرمان دقیق `npm run test:app -- --grep "T9-B1 live Brief|T9-B2|T9-B3"` سپس روی candidate پیش از final whole-slice review برابر ۵۶/۵۶ در ۳٫۷ دقیقه و بدون failure/retry پاس شد. final review سه Important محدود در فوکوس resolve/reopen Brief، فوکوس اقدام مشترک File/Source و copy غیرقابل‌اثبات checkpoint یافت؛ scoped rereview نیز follow-on همان focus finding را آشکار کرد که target fingerprint drift همان پروژه را pin نمی‌کرد. intent نهایی به exact target version/fingerprint bind شد؛ focused repairها ۴/۴، race repeat برابر ۲۰/۲۰ و target-drift برابر ۱/۱ پاس شدند. authority/UI final reruling هر دو SPEC/QUALITY PASS، Critical صفر و Important صفر هستند؛ یک Minor پوششیِ نبود E2E مستقل failure branch در `BriefInputsSection` non-blocking پذیرفته شد. ۵۶/۵۶ receipt bytes پس از این review-fixها نیست؛ full gate یک‌بارهٔ بعد از freeze شاهد قطعی خواهد بود.
...
17. نخستین گیت انتشار BG-F6 با ۶ شکست از ۴۶۳ receipt نساخت؛ یک regression عددی production و oracleهای قدیمی v1 اصلاح شدند. کد نهایی BG-F6 اکنون روی local/GitHub `main@7cab540ffad3e501e76a75c08a0e6462e36b7cea` هم‌تراز است؛ REL-2 وضعیت میزبانی بیرونی آن را دوباره ممیزی نکرد.
18. ماهیار سپس سریع‌سازی گیت را مجاز کرد. REL-2 بدون تغییر محصول، مجموعهٔ ۴۶۳تایی را به اجرای دو-worker اپ + runtime سریال منتقل کرد؛ دو پایلوت bytes یکسان ۴۶۳/۴۶۳ در `15:53.42` و `15:43.72` پاس شدند و fallback سریال حفظ شد. پس از مشاهدهٔ نتیجه، او commit/push و انتشار exact همین candidate در GitHub، Cloudflare Pages و ChatGPT Sites را نیز صریحاً مجاز کرد؛ ثبت این مجوز یک گیت تازه روی freeze نهایی را لازم کرد.
19. پس از بسته‌شدن REL-2 و مجوز جداگانه، BG-F7 در checkout اصلی به‌صورت محلی و تست‌محور اجرا شد؛ سابقهٔ evidence و وضعیت انتشار آن در بخش همنام پایین این هنداف حفظ شده است.
20. T9-B1 از fixture جدا شد، سه بخش واقعی و فقط‌خواندنی Brief را ساخت و با SHA دقیق `f77fb5a22985069028fdaecb0e324375b819ef7b` در GitHub، Cloudflare Pages و ChatGPT Sites owner-only same-source منتشر شد.
21. ماهیار سپس انجام دو تسک بعدی و انتشار مشترکشان را صریحاً خواست. T9-B2 sidecar تعیین‌تکلیف اسناد/intakeها و T9-B3 checkpoint صریح آخرین مراجعه را با دو authority مستقل، project-scoped، SHA-256 و fail-close ساختند؛ بازکردن Brief همچنان read-only است.
22. implementation base تا `cb3a619225acfb821abe61dde6abdc4f41c8aee3` و review-fixهای آن پیاده‌سازی شد. نخستین focused freeze برابر ۵۴/۵۵ روی race focus متوقف شد؛ پس از REDهای قطعی early-rAF و stale-focus دو‌تب، repair نخست بسته‌های ۲/۲ و ۵/۵ و سپس candidate پیش از final whole-slice review اجرای ۵۶/۵۶ بدون retry/failure داشت. final review سه Important محدود focus/copy و scoped rereview یک follow-on همان focus class را پیش از گیت گرفتند؛ binding exact target version/fingerprint و copy صادقانه repair شدند و evidence نهایی repair برابر ۴/۴ + race repeat ۲۰/۲۰ + target-drift ۱/۱ است. authority/UI final reruling هر دو PASS با Critical/Important صفرند؛ Minor پوششی نبود E2E مستقل failure branch در `BriefInputsSection` non-blocking پذیرفته شد. سه سند bytes نهایی را برای یک release gate منجمد می‌کنند؛ انتشار هنوز انجام نشده و ۵۶/۵۶ قبلی receipt exact final bytes نیست.
23. M1a فقط پس از PASS همان gate معماری و با پیام جدا، به‌صورت adapter محلی، opt-in، provider-neutral و text-only آزموده می‌شود؛ M1b برای `ContextManifest`/Memory فقط بعد از پذیرش M1a می‌آید.
24. مسیر تأمین‌کننده، shared case واقعی، backend و شبکهٔ واقعی همچنان به gate و تأییدهای جدا نیاز دارند.

هر بار فقط یک بستهٔ کوچک و صریح ساخته می‌شود. GitHub baseline جاری `origin/main@f77fb5a22985069028fdaecb0e324375b819ef7b` است؛ implementation base T9-B2/T9-B3 روی `cb3a619225acfb821abe61dde6abdc4f41c8aee3` قرار دارد و repairهای محدود final review همراه همین سه سند freeze نهاییِ pre-release را می‌سازند. مجوز فعلی فقط یک گیت کامل و انتشار exact همین candidate مشترک را پوشش می‌دهد؛ این کار PASS معماری، remediation بعدی یا مجوز مدل/backend/network/send/Report/Notification/supplier path نیست.
...
3. `CHIDA-PRODUCT-LEARNINGS-FA.md` را، به‌ویژه C-007 تا C-010، T7/T8، T8-UX1/T8-UX2/T8-UX3، H1، H2، MC-1، SI-1، TM-1، REL-1، REL-2، BL-1، BG-GATE-1، BG-F1 تا BG-F7 و T9-B1/T9-B2/T9-B3 بخوان. سپس `CHIDA-BUILDER-PROTOTYPE-ARCHITECTURE-GATE-FA.md` را کامل بخوان. تصمیم سه‌ورودی C-009 تاریخی و منسوخ است؛ قرارداد جاری افزودن پروژه C-010 است و قاعدهٔ Quick Action grid در C-010 با تصمیم صریح T8-UX3 منسوخ شده است.
4. `CHIDA-SOL-HANDOFF-FA.md`، بخش‌های جاری/تاریخی همین هنداف و design/plan تأییدشدهٔ `2026-09-03-t9-b2-b3-brief-authorities` را بررسی کن؛ سپس `git status --short --branch`، `git diff --check`، `git log -1` و هم‌ترازی `origin/main` را فقط read-only تطبیق بده. GitHub baseline منتشرشدهٔ فعلی `f77fb5a22985069028fdaecb0e324375b819ef7b` و implementation base پیش از repair/freeze اسناد `cb3a619225acfb821abe61dde6abdc4f41c8aee3` است؛ وضعیت Cloudflare/Sites را فقط از رسید terminal معتبر بگیر و از هم‌ترازی Git استنباط نکن.
...
6. MC-1، SI-1/TM-1، baseline تا BG-F5 و T9-B1 با رسیدهای تاریخی خود منتشرند. T9-B2/T9-B3 دو sidecar تازه با کلیدهای exact `chida-prototype-project-input-dispositions:v1` و `chida-prototype-project-visit-checkpoints:v1` هستند و تا پیش از receipt گیت/انتشار فقط local/private و unpublished محسوب می‌شوند. بازکردن Brief هیچ‌یک را نمی‌نویسد؛ فقط resolve/reopen و baseline/mark-seen صریح writer هستند.
7. قرارداد Fast Publish را حفظ کن: ابتدا bytes نهایی را freeze کن؛ `gate:release` را دقیقاً یک بار روی همان candidate اجرا کن؛ فقط پس از PASS، exact gated bytes را commit کن؛ سپس `gate:publish`، یک push و یک release same-source از همان SHA انجام بده. مجوز exact این انتشار قبلاً دریافت شده است. receipt terminal و شناسه‌های پویا فقط در پیام تحویل می‌آیند و هیچ commit یا deployment دوم برای ثبت آن‌ها ساخته نمی‌شود.
...
9. تسک جاری فقط release بستهٔ مشترک T9-B2/T9-B3 است. ابتدا کد/تست/سه سند روی bytes نهایی freeze می‌شوند؛ سپس همان مجوز موجود دقیقاً یک full gate را پوشش می‌دهد و فقط در صورت PASS، exact gated bytes commit می‌شوند، `gate:publish` اجرا می‌شود و همان SHA در GitHub `main`، Cloudflare Pages و ChatGPT Sites owner-only منتشر می‌شود. rerun Builder Gate، remediation یا تسک محصولی بعدی و عبور به مدل/backend/network/send/Report/Notification/supplier path همچنان خارج از دامنه‌اند.
...
- **T9-B1 منتشرشده؛ T9-B2/T9-B3 محلی پیاده‌سازی و بازبینی‌شده و در انتظار انتشار مشترک:** Brief پنج دستهٔ واقعی پروژه را با projection فقط‌خواندنی، disposition صریح اسناد/intakeها و checkpoint صریح آخرین مراجعه نشان می‌دهد. دو sidecar تازه local/private و project-scoped هستند؛ open/reload/focus writer نیست و خطای هر بخش مستقل fail-close می‌شود. تا رسید موفق همان SHA در هر سه مقصد، T9-B2/T9-B3 «منتشرشده» یا terminal-complete محسوب نمی‌شوند.
- **باقی‌ماندهٔ جدا:** موعد پاسخ، انقضای پیشنهاد، درخواست بی‌پاسخ و آستانهٔ زمان؛ پایش هزینه فقط پس از وجود دادهٔ مالی T12 فعال شود. دلیل، آخرین اجرا، اجرای بعدی، خاموش‌کردن، توقف، شکست و retry باید دیده شوند.
- تا نبود worker/backend فقط «بررسی هنگام بازشدن مرکز کارها یا وقتی همان مرکز در صفحهٔ باز و قابل‌دیدن است» ادعا شود؛ نه اجرای تب بسته/پنهان یا پس‌زمینهٔ دائمی. T9-B1 تا T9-B3 هیچ Report، Notification یا اجرای پس‌زمینه‌ای اضافه نکرده‌اند.
...
> این تسک ادامهٔ مستقیم پروژهٔ CHIDA در checkout اصلی `/Users/mahyarkl/Desktop/ChatGPT/CHIDA` و شاخهٔ `main` است؛ worktree جدا نساز. ابتدا `AGENTS.md`، `prototype/AGENTS.md`، `CHIDA-CONTINUATION-HANDOFF-FA.md`، design/plan `2026-09-03-t9-b2-b3-brief-authorities`، بخش T9-B1/T9-B2/T9-B3 انتهای `CHIDA-PRODUCT-LEARNINGS-FA.md`، سند مادر و backlog را بخوان. baseline منتشرشده `origin/main@f77fb5a22985069028fdaecb0e324375b819ef7b` و implementation base پیش از repair/freeze اسناد `cb3a619225acfb821abe61dde6abdc4f41c8aee3` است؛ candidate مشترک T9-B2/T9-B3 هنوز unpublished است اما مجوز exact publication دارد. bytes نهایی را freeze کن و دقیقاً یک‌بار `gate:release` اجرا کن؛ فقط در صورت PASS، exact gated bytes را commit و با `gate:publish` کنترل کن، سپس یک push و یک release همان SHA در GitHub، Cloudflare Pages و Sites owner-only انجام بده. رسیدهای پویا را فقط در پیام terminal بده. Builder Gate کامل همچنان `FAIL` و rerun نشده است؛ تسک محصولی بعدی، مدل، backend، Report/Notification، sync، شبکه/ارسال و مسیر تأمین‌کننده خارج از دامنه‌اند.
...
## BG-F6 — Proposal Authority & Concurrency Foundation؛ سابقهٔ candidate پیش از baseline کد `7cab540` — ۲۰۲۶/۰۹/۰۲
...
- **وضعیت شواهد تاریخی:** اجرای نهایی یک‌جای suite BG-F6 پیش از گیت برابر ۵۳/۵۳ و بستهٔ نمایندهٔ downstream برابر ۹/۹ پاس شد. نخستین گیت سپس build/integrity/TypeScript را پاس کرد اما با ۶ شکست Playwright از ۴۶۳ receipt نساخت. پس از اصلاح حد canonical Proposal و انتقال oracleهای غیرمهاجرتی به raw v2، همان شش مسیر ۶/۶ و بستهٔ عمیق T7-B1/T7-B2/T8-A1 تا T8-A4/T8-UX1/BG-F6 برابر ۹۱/۹۱ پاس شد؛ تست oversized، tamper، rollback، concurrency و lineage داخل آن سبز بود. این وضعیت freeze پیش از commit اکنون با baseline کد `7cab540ffad3e501e76a75c08a0e6462e36b7cea` supersede شده است.
- **Git و مرز جاری:** local/GitHub `HEAD` و `origin/main` در آغاز REL-2 روی `7cab540ffad3e501e76a75c08a0e6462e36b7cea` هم‌تراز بودند. وضعیت Cloudflare Pages و ChatGPT Sites این SHA در REL-2 دوباره ممیزی نشده و از Git استنباط نمی‌شود. مجوز قدیمی BG-F6 به Gate معماری، اصلاح بعدی، مدل، backend، شبکه/ارسال یا supplier path گسترش نداشت؛ مجوز تازه فقط commit/push و انتشار exact REL-2 پس از گیت تازه را پوشش می‌دهد.
...
## REL-2 — Release Gate Acceleration؛ انتشار exact مجاز — ۲۰۲۶/۰۹/۰۲

- **مسئله و انتخاب:** critical path گیت، ۴۵۵ تست سریال `chida-flow.spec.ts` بود؛ نه Cloudflare، Sites، build یا معماری monolith. راه‌حل محدود این برش، scheduling سطح تست با `fullyParallel` و سقف ثابت دو worker برای اپ است. هشت تست gesture/runtime با یک worker می‌مانند؛ microservice، split محصول یا تغییر deployment وارد نشد.
- **پوشش و fallback:** discovery همچنان دقیقاً ۴۵۵ اپ + ۸ runtime = ۴۶۳ است. هیچ test حذف/skip نشد، retry و timeout بالا نرفت و assertion یا کد محصول ضعیف نشد. `npm run test:all` مسیر سریع و `npm run test:all:compat` همان scheduler قدیمی برای fallback/تشخیص است؛ `test:app` و `test:runtime` نیز دست‌نخورده‌اند.
- **پایداری oracle:** هفت محل oracle قدیمی—که به‌علت یک حلقه در مجموع نه case را پوشش می‌دهند—پس از mutation ناهم‌زمان storage را پیش از terminal state می‌خواندند. هر محل ابتدا در serial/parallel طبقه‌بندی و زیر load قرمز شد؛ سپس فقط انتظار شرطیِ state نهایی موجود UI اضافه شد. بستهٔ async نهایی ۴۵/۴۵ و تکرار proposal برابر ۲۰/۲۰ پاس شد.
- **پایلوت‌های bytes یکسان:** پایلوت اول اپ `455/455` در `944.76s` و runtime `8/8` در `8.66s`، مجموع `15:53.42`؛ پایلوت دوم بدون تغییر فایل میان اجراها اپ `455/455` در `931.47s` و runtime `8/8` در `12.25s`، مجموع `15:43.72`. هر دو صفر fail/retry و در برابر مبنای `30.3m` حدود ۴۷٫۶٪ و ۴۸٫۱٪ سریع‌ترند.
- **وضعیت Git و انتشار:** baseline شروع `main@7cab540ffad3e501e76a75c08a0e6462e36b7cea` با `origin/main` هم‌تراز بود. REL-2 هنگام freeze این متن هنوز local/uncommitted/unpublished است. ماهیار commit/push و انتشار exact آن در GitHub، Cloudflare Pages و ChatGPT Sites را مجاز کرده است. receipt گیت پیشین به‌علت ثبت الزامی همین مجوز stale است؛ اسناد و tooling اکنون freeze می‌شوند و یک `npm run gate:release` تازه لازم است. پس از PASS هیچ tracked byte ویرایش نمی‌شود و فقط همان candidate با `gate:publish`، یک push و همان SHA منتشر خواهد شد؛ fingerprint، زمان و رسیدهای واقعی فقط در پیام تحویل terminal می‌آیند.
- **گام بعدی پس از تحویل:** پس از رسید موفق سه مقصد، REL-2 بسته و repo برای انتخاب یک تسک سازندهٔ بعدی آماده است؛ خود آن تسک با پیام صریح جدا آغاز می‌شود. split فیزیکی فایل تست نیز تا وقتی دو-worker معیار پایداری و زمان را پاس می‌کند، deferred می‌ماند.

## BG-F7 — Comparison Authority & Concurrency Foundation؛ candidate محلی — ۲۰۲۶/۰۹/۰۳

- **دامنه و مالکیت فایل:** `prototype/src/builderProposalComparisons.ts` مالک schema و parser exact، مشتق‌سازی جداگانهٔ Product/Service، دو authority canonical v2، SHA-256، migration/cutover، command/receipt، shared-lock mutation، readback و rollback است. `prototype/src/builderProposals.ts` فقط دو seam خالص و بدون write را export می‌کند: currentness هر Proposal record و اعتبار canonical snapshot کامل ProposalRevision؛ `prototype/src/procurementDispatch.ts` فقط primitive موجود SHA-256 را export می‌کند. `prototype/src/Prototype.tsx` مالک assembly وابستگی جاری، دو React state مستقل، editor binding/attempt، reconciliation و adapterهای downstream/UI است؛ `prototype/tests/chida-flow.spec.ts` oracle مستقل و regressionها را نگه می‌دارد. Decision و Negotiation همچنان store و Domain Object مستقل‌اند.
- **توپولوژی هشت کلید:** خانوادهٔ Product از legacy `chida-prototype-builder-proposal-comparisons:v1`، canonical `chida-prototype-builder-proposal-comparisons:v2`، marker `chida-prototype-builder-proposal-comparisons:v2:cutover:v1` و incident `chida-prototype-builder-proposal-comparisons:v2:rollback-incident:v1` استفاده می‌کند. خانوادهٔ Service به‌ترتیب `chida-prototype-builder-service-proposal-comparisons:v1`، `chida-prototype-builder-service-proposal-comparisons:v2`، `chida-prototype-builder-service-proposal-comparisons:v2:cutover:v1` و `chida-prototype-builder-service-proposal-comparisons:v2:rollback-incident:v1` را دارد. authorityها مستقل‌اند ولی هر دو از همان Web Lock مشترک procurement استفاده می‌کنند.
- **مرز سازگاری:** v1 فقط ورودی migration صریح است و canonical/marker/incident ناخوانا fallback یا overwrite نمی‌سازد. Decision محصول، Decision خدمت، `NegotiationDraft`، `ManualNegotiationResponse`، `ResponseReview` و `ConditionImpact` در cutover نوشته، re-key یا normalize نمی‌شوند؛ target تاریخی FNV فقط با evidence دقیق همان Comparison revision resolve می‌شود، descendant تازه SHA canonical را pin می‌کند و update descendant قدیمی target قبلی را byte-stable نگه می‌دارد. Product و Service در parser، state و failure مستقل‌اند و منطق decimal محصول با ده معیار ثابت خدمت ادغام نمی‌شود.
- **شواهد تازهٔ پس از همهٔ review-fixها:** discovery متمرکز BG-F7 دقیقاً ۴۲ بود و اجرای سریال نهایی روی آخرین bytes برابر ۴۲/۴۲ در ۷٫۳ دقیقه، T7 برابر ۹/۹ در ۱٫۲ دقیقه و downstream منتخب برابر ۱۱/۱۱ در ۱٫۸ دقیقه، همگی بدون retry/skip پاس شدند. REDهای واقعی پیش از fix، علاوه بر مرز ۱۹۹رقمی × ۱۰۰٪ و receipt replay پس از historical شدن Contact، mutation تازه از stale attempt بدون receipt، بازمرتب‌شدن کلیدهای `"10"/"2"` توسط JSON، شکست replay پروژهٔ معتبر `__proto__` و پذیرفته‌شدن ورودی nested غیر-JSON را اثبات کردند؛ مسیرهای اصلاح‌شده در production و oracle مستقل سبز شدند. replayهای Contact و project-switch اکنون canonical/marker/incident را byte-for-byte ثابت می‌سنجند و preview مرزی خود عدد live را نیز اثبات می‌کند. اجرای پشتیبانی‌شدهٔ `(cd prototype && npm exec -- tsc --noEmit)`، `npm --prefix prototype run check:runtime` برای هر ۲۸ فایل، build نهایی همراه Sites preparation و diff-check tracked/untracked پاس شدند؛ تنها warning build همان chunk-size شناخته‌شده بود. فرم اولیه/تاریخی `npm --prefix prototype exec -- tsc --noEmit` فقط TypeScript help چاپ کرده بود و receipt نیست؛ attempt موازی downstream با teardown پورت مشترک نیز receipt نبود و اجرای سریال ۱۱/۱۱ جای آن را گرفت. QA گستردهٔ واقعی Vite در `390 × 844` پیش از آخرین سخت‌گیری‌های hash/replay، Product/Service، stale/fault/isolation/disclosure، overflow صفر، console صفر و request خارجی صفر را پوشش داد؛ تمام runtime pathهای متأثر پس از آن نیز با Playwright واقعی همان viewport دوباره سبز شدند. این evidence focused candidate است، نه receipt یک release gate کامل.
- **اصلاح متمرکز blocker گیت:** نخستین full-gate تازه پس از build روی تست قدیمی Mock در case ۱۰۳ از ۴۹۷ متوقف و برای صرفه‌جویی در زمان قطع شد؛ receipt ساخته نشد. همان failure در اجرای سریال ۱/۱ بازتولید شد و trace نشان داد snapshot تست پیش از تکمیل cutover پس‌زمینهٔ Product/Service Comparison گرفته می‌شد؛ صفحهٔ Mock writer نبود. oracle بدون تغییر رفتار محصول، پیش از snapshot منتظر markerهای committed هر دو خانواده می‌ماند؛ اجرای سریال ۱/۱ و تکرار موازی ۱۰/۱۰ پاس شد. چون bytes تست و اسناد تغییر کردند، فقط یک full gate تازه روی freeze نهایی معتبر خواهد بود.
- **وضعیت Git و انتشار:** GitHub baseline فعلی `origin/main@5df0ef6e34713d999143cc313438fbb9e4996613` و local `HEAD@ef951657c3a9c41ba0a6269596de1340f715a2a8` است. پایهٔ design BG-F7 در همان commit محلی است؛ amendment design، implementation، plan و سه سند زنده در working tree اصلی‌اند و exact surface ده‌فایلی پس از review مستقل بدون finding Critical/Important/Minor منجمد است. کل BG-F7 unpublished است و هیچ `gate:release` کامل، commit تازه، push، Cloudflare deployment یا Sites publication برای BG-F7 انجام نشده است. نتیجهٔ تاریخی `BG-GATE-1` همچنان `FAIL` است و این برش مدل، backend، شبکه/ارسال، `case_private` یا مسیر تأمین‌کننده را مجاز نمی‌کند.
- **بدهی اعلام‌شده:** بازسنجی commit-time currentness در writer فعلی Product Decision به‌اندازهٔ مسیر Service یک برش مستقل بعدی است. reader فعلی Negotiation نیز هر دو Comparison family را با هم می‌خواند؛ بنابراین خرابی یک family می‌تواند Negotiation را موقتاً unavailable کند، هرچند ledger سالم Comparison مستقل می‌ماند. این موارد داخل scope یا completion BG-F7 نیستند؛ canonicalization/concurrency خود Decision/Negotiation، File/Photo و سایر تصمیم‌های Gate نیز باز می‌مانند.
- **تنها گام بعدی مجاز:** از ماهیار مجوز صریح اجرای یک full gate و انتشار همین exact candidate ده‌فایلی در مقصدهای دقیق گرفته شود؛ تنها در صورت PASS همان candidate با یک commit/یک SHA در GitHub `main`، Cloudflare Pages و ChatGPT Sites owner-only منتشر می‌شود. طراحی/اجرای BG-F7 یا عبارت عمومی «ادامه بده» جای این مجوز exact publication نیست.

## T9-B1 — Live Brief Read Projection؛ منتشرشده — ۲۰۲۶/۰۹/۰۳

- **release بسته‌شده:** بریف از state موجود و ازپیش‌فیلترشدهٔ پروژهٔ فعال، سه projection «تأییدهای منتظر»، «کارهای باز/موعدها» و «درخواست‌های خرید» می‌سازد. موعد آزاد Task دستی فقط «موعد ثبت‌شده» است و طبقه‌بندی عقب‌افتاده/آینده فقط از `dueAt` canonical Task برنامه می‌آید. وضعیت هر بخش مستقل است و بازکردن Brief هیچ storage write ندارد.
- **مبنای جاری:** T9-B1 پس از gate با SHA دقیق `f77fb5a22985069028fdaecb0e324375b819ef7b` در GitHub `main`، Cloudflare Pages و ChatGPT Sites owner-only same-source منتشر شد. این SHA اکنون `origin/main` و تنها baseline زنده برای candidate T9-B2/T9-B3 است؛ شناسه‌های پویای deployment/version در رسید terminal ماندند و در سند tracked تکرار نشدند.

## T9-B2 — Project Input Disposition؛ candidate محلی — ۲۰۲۶/۰۹/۰۴

- **authority و semantics:** store exact `chida-prototype-project-input-dispositions:v1` یک sidecar مستقل، خصوصی و project-scoped است. واحدهای canonical فقط سند غیرتصویری مستقل و intake کامل Composer هستند؛ فایل وابستهٔ intake دوبار شمرده نمی‌شود و عکس مستقل گالری خارج است. target ماندگار exact فقط `{ kind, id, projectId, version: 1, fingerprint }` است؛ title/meta/زمان و مقصد File یا Source مشتق و UI-only می‌مانند.
- **اقدام صریح و read-only:** نبود disposition برای target موجود یعنی pending؛ فقط «تعیین‌تکلیف شد» revision resolved و «بازکردن دوباره» revision pending می‌سازند. تغییر target حل‌شده بدون mutation به `pending-stale` برمی‌گردد. بازکردن Brief، reload، focus، storage event و تعویض پروژه writer نیستند. سند `metadata-only` قابل‌تعیین‌تکلیف است اما هیچ ادعای bytes، اصالت یا content verification ندارد؛ `browser-file` فقط پس از reconciliation سالم آمادهٔ mutation است.
- **مرز شکست و هم‌زمانی:** parser exact و replay، SHA-256، binding هویت/اختیار، Web Lock اختصاصی، expected store/record version، idempotency، commit-time reread، exact readback و rollback فقط برای candidate-owned bytes اعمال می‌شوند. dependency ناخوانا/مفقود، scope/policy drift، stale target، race دو تب یا rollback تأییدنشده fail-close می‌شود؛ نتیجهٔ نامعلوم post-write موفقیت یا حفظ baseline را جعل نمی‌کند. File/Source و assetهای موجود read-only dependency هستند و schema/bytes آن‌ها تغییر نمی‌کند.
- **UI و استقلال failure:** چهارمین بخش Brief حداکثر سه مورد را بعد از sort قطعی stale سپس pending، زمان نزولی و tie-break `kind + id` نشان می‌دهد و هر مورد دقیقاً File detail یا Source detail خودش را باز می‌کند. خرابی sidecar فقط همین بخش و input headهای T9-B3 را unavailable می‌کند؛ سه بخش سالم T9-B1 باقی می‌مانند.

## T9-B3 — Explicit Project Visit Checkpoint؛ candidate محلی — ۲۰۲۶/۰۹/۰۴

- **authority و تعریف مراجعه:** store exact `chida-prototype-project-visit-checkpoints:v1` sidecar مستقل، خصوصی و project-scoped است و هر پروژه حداکثر یک checkpoint نسخه‌دار دارد. «آخرین مراجعه» فقط commit موفق «ثبت وضعیت فعلی به‌عنوان مبنا» یا «همه را دیدم» است؛ بازکردن Brief، Drawer، reload، focus، storage event و schedule محلی هیچ checkpointی نمی‌نویسند. اولین استفاده delta یا عدد ساختگی نمایش نمی‌دهد.
- **دامنهٔ تغییر:** observation فقط headهای exact `manual-task`، `backbone-task`، `content-approval`، `dispatch-plan-approval`، `purchase-request` و `project-input` را با SHA-256 مستقل و ترتیب `kind + id` می‌گیرد. UI فقط شمار `added/updated` چهار گروه «کارها»، «تصمیم‌ها»، «خریدها» و «اسناد و ورودی‌ها» را نشان می‌دهد؛ ranking/AI وجود ندارد. head حذف‌شده از baseline بدون tombstone معتبر `removed` تلقی نمی‌شود و کل delta/mark-seen را unavailable می‌کند.
- **مرز شکست و هم‌زمانی:** parser/replay exact، هویت/اختیار، Web Lock checkpoint، expected version، idempotency، reread همهٔ dependencyها داخل lock، تطبیق observation کلیک‌شده، exact readback و candidate-owned rollback برقرار است. dependency یا checkpoint ناخوانا، stale observation، رقابت دو تب و شکست write/readback/rollback baseline یا `observedAt` را جلو نمی‌برد. failure checkpoint فقط بخش پنجم و CTA آن را unavailable می‌کند؛ بخش‌های سالم T9-B1/T9-B2 با دادهٔ خود باقی می‌مانند.
- **ناوبری و دسترس‌پذیری:** گروه‌ها در همان مرکز واحد «کارها» یا «خریدها» تمام وضعیت‌های تغییرکرده را نشان می‌دهند؛ منشأ بازگشت به همان launch bind است و در navigation تو‌در‌تو به بازدید بعدی نشت نمی‌کند. بازگشت از detail فیلتر aggregate را حفظ می‌کند و focus مقصد/بازگشت، target حداقل ۴۴px، scroll واقعی و overflow افقی صفر پوشش دارند.

## freeze مشترک T9-B2/T9-B3 و تنها اقدام بعدی

- **scope candidate:** نسبت به baseline `f77fb5a22985069028fdaecb0e324375b819ef7b` فقط design/plan تأییدشده، `prototype/src/projectBriefAuthorities.ts`، اتصال‌های app-owned در `prototype/src/Prototype.tsx` و `prototype/src/prototype.css`، regressionهای `prototype/tests/chida-flow.spec.ts` و همین سه سند وضعیت وارد candidate شده‌اند. فایل‌های runtime محافظت‌شده، worker، hosting contract و lockfile تغییر نکرده‌اند.
- **شواهد candidate و repair نهایی:** اجرای نخست با ۵۴/۵۵ روی race focus متوقف و freeze آن candidate لغو شد. early-rAF قطعی RED برابر ۰/۱ سپس GREEN، stale-focus دو‌تب RED برابر ۰/۱ سپس بستهٔ ترکیبی ۲/۲ و checkpointهای مجاور ۵/۵ ثبت شدند. فرمان دقیق `npm run test:app -- --grep "T9-B1 live Brief|T9-B2|T9-B3"` روی candidate بعدی و پیش از final whole-slice review برابر ۵۶/۵۶ در ۳٫۷ دقیقه، بدون failure یا retry پاس شد؛ build/TypeScript/Vite/Sites preparation، `check:runtime` برای هر ۲۸ فایل، `git diff --check` و protected-runtime diff نیز روی همان candidate PASS بودند. final review سه finding Important محدود در focus/copy و scoped rereview یک follow-on همان focus class یافت: intent باید exact target version/fingerprint را نیز pin کند تا drift همان پروژه intent قدیمی را زنده نگه ندارد. binding نهایی و copy preservation-neutral repair شدند؛ focused repairها ۴/۴، race repeat برابر ۲۰/۲۰ و target-drift قطعی دوصفحه‌ای ۱/۱ پاس‌اند. authority/UI final reruling هر دو SPEC/QUALITY PASS، Critical صفر و Important صفر هستند. یک Minor پوششی—نبود E2E مستقل failure branch در `BriefInputsSection`—صریحاً non-blocking پذیرفته شد. ۵۶/۵۶ شاهد pre-final-review است، نه receipt exact final bytes؛ سپس یک full gate روی frozen bytes شواهد قطعی را می‌بندد. QA `390 × 844` مسیرهای resolve/reopen، baseline/delta/mark-seen، reload/project switch، partial failure، focus/scroll، RTL، overflow صفر، console/pageerror صفر و request بیرونی صفر را پوشش می‌دهد.
- **مرز صادقانه:** این بسته Report ماندگار، اعلان/Notification API، service worker/worker، backend، مدل، OCR/extraction، وب/شبکه، sync/share، اجرای تب بسته/پنهان، مسیر تأمین‌کننده یا اثر بیرونی نمی‌سازد. برنامهٔ daily/weekly همان شبیه‌سازی local قبلی است. سند مادر تغییر نکرده و پیشنهاد learning فقط ورودی بازنگری آینده است. `BG-GATE-1` rerun نشده و نتیجهٔ تاریخی آن همچنان `FAIL` است.
- **مجوز و توقف:** ماهیار اجرای دو تسک و انتشار مشترکشان را صریحاً مجاز کرده است، ولی این candidate هنوز local/unpublished و بدون receipt است. ابتدا exact bytes نهایی freeze می‌شوند و فقط یک `npm run gate:release` کامل روی همان bytes اجرا می‌شود؛ در صورت PASS، exact gated bytes commit می‌شوند، سپس `gate:publish` و یک push همان SHA مبنای GitHub `main`، Cloudflare Pages و ChatGPT Sites owner-only می‌شوند. شناسه‌های پویا فقط در پیام تحویل terminal گزارش می‌شوند و پیش از وجودشان در سند درج نمی‌شوند. هیچ تسک محصولی بعدی پیش از رسید موفق هر سه مقصد آغاز نمی‌شود.
