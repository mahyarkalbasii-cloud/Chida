...
**تاریخ آخرین به‌روزرسانی:** ۱۴۰۵/۰۶/۱۲ — ۲۰۲۶/۰۹/۰۳
**مسیر پروژه:** `/Users/mahyarkl/Desktop/ChatGPT/CHIDA`
**نقطهٔ ادامه:** REL-2 اکنون روی GitHub `origin/main@5df0ef6e34713d999143cc313438fbb9e4996613` قرار دارد؛ وضعیت Cloudflare Pages و ChatGPT Sites آن در این به‌روزرسانی دوباره ممیزی نشده و از Git استنباط نمی‌شود. checkout اصلی روی `main@ef951657c3a9c41ba0a6269596de1340f715a2a8` یک commit پایهٔ طراحی BG-F7 جلوتر از `origin/main` است؛ amendment همان design همراه implementation نهایی `BG-F7 — Comparison Authority & Concurrency Foundation`، plan و سه سند در working tree قرار دارد. exact candidate ده‌فایلی BG-F7 پس از evidence تازه و review مستقل بدون finding باز منجمد است. کل BG-F7 هنوز local/unpublished است؛ نه `gate:release` کامل اجرا شده، نه commit تازه/push/deploy یا publication انجام شده است. `BG-GATE-1` تاریخی همچنان `FAIL` و rerun نشده است.

## مأموریت گفت‌وگوی بعدی

**نقطهٔ توقف جاری:** پیاده‌سازی، evidence نهایی، بازبینی مستقل و فریز exact candidate ده‌فایلی BG-F7 کامل شده‌اند. تنها اقدام بعدی درخواست مجوز صریح اجرای یک `gate:release` کامل و—فقط در صورت PASS—commit/push و انتشار همان SHA در GitHub `main`، Cloudflare Pages و ChatGPT Sites owner-only است. تا دریافت این مجوز، گیت، commit، push، deployment/publication و هر تسک محصولی بعدی متوقف‌اند. مدل، backend، شبکه/ارسال، `case_private` و مسیر تأمین‌کننده نیز خارج از دامنه می‌مانند.
...
19. پس از بسته‌شدن REL-2 و مجوز جداگانه، BG-F7 در checkout اصلی به‌صورت محلی و تست‌محور اجرا شد. دو ledger مستقل Product/Service Comparison به authority v2 منتقل شدند و شواهد تازهٔ آن discovery برابر ۴۲، focused برابر ۴۲/۴۲، T7 برابر ۹/۹، downstream برابر ۱۱/۱۱ و PASS TypeScript/runtime/build/diff است؛ candidate هنوز uncommitted/unpublished و بدون release gate کامل است.
20. M1a فقط پس از PASS همان gate معماری و با پیام جدا، به‌صورت adapter محلی، opt-in، provider-neutral و text-only آزموده می‌شود؛ M1b برای `ContextManifest`/Memory فقط بعد از پذیرش M1a می‌آید.
21. مسیر تأمین‌کننده، shared case واقعی، backend و شبکهٔ واقعی همچنان به gate و تأییدهای جدا نیاز دارند.

هر بار فقط یک تسک کوچک ساخته می‌شود. GitHub baseline جاری `origin/main@5df0ef6e34713d999143cc313438fbb9e4996613` و local `HEAD@ef951657c3a9c41ba0a6269596de1340f715a2a8` است؛ BG-F7 در working tree محلی، ثبت‌نشده و منتشرنشده قرار دارد. کار مجاز فعلی فقط freeze دقیق، بازبینی مستقل و درخواست مجوز صریح انتشار همان candidate است؛ این کار PASS معماری، remediation بعدی یا مجوز مدل/backend/network/send/supplier path نیست.
...
3. `CHIDA-PRODUCT-LEARNINGS-FA.md` را، به‌ویژه C-007 تا C-010، T7/T8، T8-UX1/T8-UX2/T8-UX3، H1، H2، MC-1، SI-1، TM-1، REL-1، REL-2، BL-1، BG-GATE-1 و BG-F1 تا BG-F7 بخوان. سپس `CHIDA-BUILDER-PROTOTYPE-ARCHITECTURE-GATE-FA.md` را کامل بخوان. تصمیم سه‌ورودی C-009 تاریخی و منسوخ است؛ قرارداد جاری افزودن پروژه C-010 است و قاعدهٔ Quick Action grid در C-010 با تصمیم صریح T8-UX3 منسوخ شده است.
4. `CHIDA-SOL-HANDOFF-FA.md`، بخش تاریخی MC-1، بخش‌های جاری SI-1/TM-1 و بخش‌های REL-1/REL-2/BL-1/BG-GATE-1/BG-F1 تا BG-F7 همین هنداف را کامل بخوان؛ سپس plan و design BG-F7 را بررسی و `git status --short --branch`، `git diff --check`، `git log -1` و هم‌ترازی `origin/main` را فقط read-only تطبیق بده. GitHub baseline فعلی `5df0ef6e34713d999143cc313438fbb9e4996613` و local `HEAD` فعلی `ef951657c3a9c41ba0a6269596de1340f715a2a8` است؛ وضعیت Cloudflare/Sites را فقط از آخرین رسید terminal معتبر بگیر و از هم‌ترازی Git استنباط نکن.
...
6. MC-1، SI-1/TM-1 و baseline تا BG-F5 با رسیدهای تاریخی خود منتشرند. BG-F6 با commit `7cab540ffad3e501e76a75c08a0e6462e36b7cea` و REL-2 با commit `5df0ef6e34713d999143cc313438fbb9e4996613` در تاریخچهٔ local/GitHub قرار دارند، اما وضعیت دو مقصد میزبانی باید از رسید بیرونی معتبر و جداگانه خوانده شود. پایهٔ design BG-F7 در commit محلی و amendment/implementation/اسناد آن در working tree است؛ کل BG-F7 unpublished و بدون full release gate یا مجوز exact publication است.
7. قرارداد Fast Publish را حفظ کن: `gate:release` یک بار روی candidate نهایی، سپس فقط پس از مجوز انتشار commit همان bytes، `gate:publish`، یک push و یک release same-source؛ receipt terminal فقط در پیام تحویل. داخل گیت، ۴۵۵ تست اپ با دو worker و ۸ runtime سریال‌اند؛ `test:all:compat` fallback scheduler قبلی است.
...
9. تسک جاری فقط بستن هنداف محلی `BG-F7 — Comparison Authority & Concurrency Foundation` است. اقدام بعدی به freeze candidate، بازبینی مستقل و درخواست مجوز exact publication محدود است؛ `gate:release`، commit/push، Cloudflare/Sites، rerun Builder Gate، remediation یا تسک محصولی بعدی و عبور به مدل/backend/network/send/supplier path همچنان پیام صریح جدا می‌خواهند.
...
> این تسک ادامهٔ مستقیم پروژهٔ CHIDA در checkout اصلی `/Users/mahyarkl/Desktop/ChatGPT/CHIDA` و شاخهٔ `main` است؛ worktree جدا نساز. ابتدا `AGENTS.md`، `prototype/AGENTS.md`، `CHIDA-CONTINUATION-HANDOFF-FA.md`، بخش‌های H2/MC-1/SI-1/TM-1/REL-1/REL-2/BL-1/BG-GATE-1/BG-F1 تا BG-F6 و بازخوردهای انتهایی `CHIDA-PRODUCT-LEARNINGS-FA.md`، سند مادر و backlog را بخوان. baseline پیش از REL-2 روی local/GitHub `main@7cab540ffad3e501e76a75c08a0e6462e36b7cea` بود؛ نتیجه و fingerprint گیت REL-2 و وضعیت سه مقصد را فقط از پیام terminal تحویل بگیر و جداگانه read-only تطبیق بده. تا پیام بعدی هیچ edit، test، gate، commit، push یا deploy انجام نده. Builder Gate کامل همچنان `FAIL` و rerun نشده است؛ remediation بعدی، مدل، backend، sync، شبکه/ارسال و مسیر تأمین‌کننده نیز مجاز نیستند.
...
## T9-B1 — Live Brief Read Projection؛ candidate محلی — ۲۰۲۶/۰۹/۰۳

- **baseline و مجوز:** پیش از شروع این برش، local/GitHub `main` تمیز و روی `02d3107593245a2b2ac41f3ae0b895e51dd1cf35` هم‌تراز بود و بررسی زندهٔ همان نوبت انتشار موفق آن را در GitHub، Cloudflare Pages و ChatGPT Sites owner-only تأیید کرد. ماهیار با «عالیه بزن بریم» اجرای سریع T9-B1 و سپس با پیام جداگانهٔ «منتشر کن» انتشار exact candidate تازه را صریحاً مجاز کرد.
- **دامنهٔ بسته:** فقط UI بریف در `prototype/src/Prototype.tsx` و `prototype/src/prototype.css` و regressionهای آن در `prototype/tests/chida-flow.spec.ts` تغییر کردند. بریف از state موجود و ازپیش‌فیلترشدهٔ پروژهٔ فعال، سه projection «تأییدهای منتظر»، «کارهای باز/موعدها» و «درخواست‌های خرید» می‌سازد. حداکثر سه مورد در هر بخش دیده می‌شود، شمار کل حفظ می‌شود و CTAها به filter تأیید/کار یا فهرست درخواست‌ها می‌روند.
- **صداقت داده:** موعد آزاد Task دستی parse نمی‌شود و فقط «موعد ثبت‌شده» است؛ طبقه‌بندی عقب‌افتاده/آینده فقط روی `dueAt` canonical Task برنامه انجام می‌شود. تغییرات از آخرین مراجعه و وضعیت اسناد چون authority ندارند محاسبه یا جعل نمی‌شوند. این برش Report، schema/store، snapshot مراجعه، backend، مدل، شبکه، worker، اعلان یا اثر بیرونی تازه ندارد.
- **partial failure و read-only:** وضعیت هر بخش مستقل است؛ failure منبع خرید/تأیید، کار سالم را empty جا نمی‌زند. بازکردن Brief هیچ writer ندارد و تست، کل `localStorage` را پیش و پس از بازشدن byte-for-byte ثابت سنجید. writer قدیمی فقط برای اقدام صریح «ذخیرهٔ برنامهٔ محلی» باقی است.
- **شواهد:** TDD ابتدا نبود projection/testidهای تازه را قرمز کرد. اجرای نهایی T9-B1 برابر ۳/۳ و بستهٔ تمام regressionهای Brief برابر ۶/۶ پاس شد؛ build شامل TypeScript/Vite/Sites prepare و runtime integrity هر ۲۸ فایل پاس است. QA واقعی `390 × 844` پروژهٔ فعال، data isolation، عقب‌افتادگی canonical، سه CTA، partial read failure، overflow افقی صفر و console warning/error صفر را پوشش داد. بازبینی مستقل finding باز P0/P1/P2 نداشت و `git diff --check` تمیز است.
- **وضعیت و تنها گام بعد:** candidate شش‌فایلی شامل سه فایل کد/تست و همین سه سند زنده اکنون مجاز انتشار اما هنوز uncommitted/unpublished است. از این نقطه bytes منجمدند: دقیقاً یک full gate اجرا می‌شود و فقط در صورت PASS همان candidate با یک commit و یک SHA در GitHub `main`، Cloudflare Pages و ChatGPT Sites owner-only منتشر خواهد شد. هیچ receipt-only edit/commit یا شروع تسک بعدی داخل این مجوز نیست.
